...
export const DataTab: React.FC<DataTabProps> = ({
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6" id="data-tab-container">
      {/* Unified Tab Header & Search Input Sticky Section */}
      <div className="sticky top-[72px] md:top-[96px] z-30 bg-slate-50/90 backdrop-blur-md -mx-1.5 px-1.5 md:-mx-4 md:px-4 py-1.5 border-b border-slate-200/80 flex flex-col gap-1.5" id="data-header-section">
        <div className="flex flex-row items-center justify-between gap-2" id="data-title-row">
        ...
        {/* Search Input (No outer wrapper card, integrated directly and elegantly) */}
        <div className="relative flex items-center" id="data-search-wrapper">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-200 text-slate-700 placeholder-slate-400 rounded-xl pl-10 pr-4 py-2 text-xs sm:text-sm font-semibold outline-none focus:ring-2 focus:ring-emerald-500/10 transition-all shadow-xs"
          />
        </div>
      </div>
      ...
      {detailGameId && (() => {
        ...
        const getHoleShotAndPutt = (score: number, idx: number, playerId: string) => {
          ...
          if (savedShots && savedPutts) {
            const s = savedShots[idx] || 0;
            const p = savedPutts[idx] || 0;
            return { shots: s, putts: p };
          }
          
          // Heuristic estimation fallback
          if (score === 1) {
            return { shots: 1, putts: 0 };
          } else if (score === 2) {
            return { shots: 1, putts: 1 };
          } else {
            return { shots: score - 2, putts: 2 };
          }
        };
            ...
            <div className="bg-white rounded-none sm:rounded-3xl max-w-2xl w-full p-3 xs:p-4 sm:p-7 shadow-2xl border-x-0 sm:border border-slate-100 my-0 sm:my-8 relative flex flex-col space-y-2 sm:space-y-3.5 animate-in slide-in-from-bottom-4 duration-300" id="game-detail-modal-card">
              ...
              <div className="space-y-4 sm:space-y-5 my-6 sm:my-8" id={`scorecards-container-${game.id}`}>
                ...
                <div className="overflow-x-auto" id={`scorecard-front-scroll-${game.id}`}>
                  <table className="w-full text-center border-separate border-spacing-0 rounded-none overflow-hidden text-[11px] sm:text-xs font-mono table-fixed" id={`scorecard-table-front-${game.id}`}>
                    <colgroup>
                      <col className="w-[11%]" />
                      <col className="w-[8.1%]" />
                      <col className="w-[8.1%]" />
                    ...
                    <thead>
                      <tr className="bg-emerald-50 text-emerald-700" id={`scorecard-front-headers-${game.id}`}>
                        <th className="py-2 px-1 text-center font-sans font-semibold text-emerald-700 border-t-2 border-b-2 border-l-2 border-r-2 border-slate-400 truncate">H</th>
                        {Array.from({ length: 9 }).map((_, idx) => (
                          <th key={idx} className={`py-2 px-0.5 font-semibold border-t-2 border-b-2 ${idx === 8 ? '' : 'border-r'} border-slate-400`}>{idx + 1}</th>
                        ))}
                        <th className="py-2 px-1 font-bold bg-emerald-50/50 text-green-700 border-t-2 border-b-2 border-l-2 border-l-slate-400 border-r-2 border-slate-400">OUT</th>
                    ...
                    <tbody>
                      ...
                      <tr className="text-slate-500 bg-white" id={`scorecard-front-par-row-${game.id}`}>
                        <td className="py-2 px-1 text-center font-sans font-medium border-b-2 border-l-2 border-r-2 border-slate-400 bg-white">Par</td>
                        {course?.pars.slice(0, 9).map((par, idx) => (
                          <td key={idx} className={`py-2 px-0.5 font-medium border-b-2 ${idx === 8 ? '' : 'border-r'} border-slate-400 bg-white text-rose-500`}>{par}</td>
                        ))}
                        <td className="py-2 px-1 font-bold bg-white text-green-600 border-b-2 border-l-2 border-l-slate-400 border-r-2 border-slate-400">
                      ...
                      <tr className={`text-slate-800 ${p1RowBg}`} id={`scorecard-front-p1-row-${game.id}`}>
                        <td className={`py-1.5 px-1 text-center font-sans font-bold text-slate-700 border-b border-l-2 border-r-2 border-slate-400 truncate uppercase ${p1NameBg}`}>
                          {p1.name.slice(0, 4)}
                        </td>
                        {p1Scores.slice(0, 9).map((score, idx) => (
                          <td key={idx} className={`p-0.5 sm:p-1 border-b ${idx === 8 ? '' : 'border-r'} border-slate-300`}>
                            <div className="flex flex-col items-center justify-center py-1 select-none">
                              ...
                              {score > 0 && (() => {
                                const stats = getHoleShotAndPutt(score, idx, p1.id);
                                return stats ? (
                                  <span className="text-[9px] sm:text-[10px] text-slate-400 font-mono mt-0.5 whitespace-nowrap leading-none">
                                    {stats.shots}/{stats.putts}
                                  </span>
                                ) : null;
                              })()}
                            </div>
                        ...
                        <td className={`py-1.5 px-1 border-b border-slate-300 border-l-2 border-l-slate-400 border-r-2 border-r-slate-400 text-center ${p1OutInBg}`}>
                          {(() => {
                      ...
                      <tr className={`text-slate-800 ${p2RowBg}`} id={`scorecard-front-p2-row-${game.id}`}>
                        <td className={`py-1.5 px-1 text-center font-sans font-bold text-slate-700 border-b-2 border-l-2 border-r-2 border-slate-400 truncate uppercase ${p2NameBg}`}>
                          {p2.name.slice(0, 4)}
                        </td>
                        {p2Scores.slice(0, 9).map((score, idx) => (
                          <td key={idx} className={`p-0.5 sm:p-1 border-b-2 ${idx === 8 ? '' : 'border-r'} border-slate-300`}>
                            <div className="flex flex-col items-center justify-center py-1 select-none">
                              <div className="w-full h-7 sm:h-8 flex items-center justify-center">
                                {renderHoleScoreBadge(score, course?.pars[idx] || 4)}
                              </div>
                              {score > 0 && (() => {
                                const stats = getHoleShotAndPutt(score, idx, p2.id);
                                return stats ? (
                                  <span className="text-[9px] sm:text-[10px] text-slate-400 font-mono mt-0.5 whitespace-nowrap leading-none">
                                    {stats.shots}/{stats.putts}
                                  </span>
                        ...
                        <td className={`py-1.5 px-1 border-b-2 border-l-2 border-l-slate-400 border-r-2 border-r-slate-400 text-center ${p2OutInBg}`}>
                          {(() => {
                ...
                <div className="overflow-x-auto" id={`scorecard-back-scroll-${game.id}`}>
                  <table className="w-full text-center border-separate border-spacing-0 rounded-none overflow-hidden text-[11px] sm:text-xs font-mono table-fixed" id={`scorecard-table-back-${game.id}`}>
                    <colgroup>
                      <col className="w-[11%]" />
                      <col className="w-[8.1%]" />
                      <col className="w-[8.1%]" />
                    ...
                    <thead>
                      <tr className="bg-emerald-50 text-emerald-700" id={`scorecard-back-headers-${game.id}`}>
                        <th className="py-2 px-1 text-center font-sans font-semibold text-emerald-700 border-t-2 border-b-2 border-l-2 border-r-2 border-slate-400 truncate">H</th>
                        {Array.from({ length: 9 }).map((_, idx) => (
                          <th key={idx + 9} className={`py-2 px-0.5 font-semibold border-t-2 border-b-2 ${idx === 8 ? '' : 'border-r'} border-slate-400`}>{idx + 10}</th>
                        ))}
                        <th className="py-2 px-1 font-bold bg-emerald-50/50 text-green-700 border-t-2 border-b-2 border-l-2 border-l-slate-400 border-r-2 border-slate-400">IN</th>
                        <th className="py-2 px-1 font-bold bg-emerald-50/50 text-red-600 border-t-2 border-b-2 border-r-2 border-slate-400">TOT</th>
                      </tr>
                    </thead>
                    <tbody>
                      {/* Par Row */}
                      <tr className="text-slate-500 bg-white" id={`scorecard-back-par-row-${game.id}`}>
                        ...
                        {course?.pars.slice(9, 18).map((par, idx) => (
                          <td key={idx + 9} className={`py-2 px-0.5 font-medium border-b-2 ${idx === 8 ? '' : 'border-r'} border-slate-400 bg-white text-rose-500`}>{par}</td>
                        ))}
                        <td className="py-2 px-1 font-bold bg-white text-green-600 border-b-2 border-l-2 border-l-slate-400 border-r-2 border-slate-400">
                          {course?.pars.slice(9, 18).reduce((a, b) => a + b, 0)}
                        </td>
                        <td className="py-2 px-1 font-bold bg-white text-red-600 border-b-2 border-r-2 border-slate-400">
                          {totalPar}
                        </td>
                      </tr>
                      {/* Player 1 Row */}
                      <tr className={`text-slate-800 ${p1RowBg}`} id={`scorecard-back-p1-row-${game.id}`}>
                        ...
                        {p1Scores.slice(9, 18).map((score, idx) => (
                          <td key={idx + 9} className={`p-0.5 sm:p-1 border-b ${idx === 8 ? '' : 'border-r'} border-slate-300`}>
                            <div className="flex flex-col items-center justify-center py-1 select-none">
                              <div className="w-full h-7 sm:h-8 flex items-center justify-center">
                                {renderHoleScoreBadge(score, course?.pars[idx + 9] || 4)}
                              </div>
                              {score > 0 && (() => {
                                const stats = getHoleShotAndPutt(score, idx + 9, p1.id);
                                return stats ? (
                                  <span className="text-[9px] sm:text-[10px] text-slate-400 font-mono mt-0.5 whitespace-nowrap leading-none">
                                    {stats.shots}/{stats.putts}
                                  </span>
                        ...
                        <td className={`py-1.5 px-1 border-b border-l-2 border-l-slate-400 border-r-2 border-r-slate-400 text-center ${p1OutInBg}`}>
                          {(() => {
                        ...
                        <td className={`py-1.5 px-1 text-center border-b border-r-2 border-slate-400 ${p1OutInBg}`}>
                          {(() => {
                            const relLabel = getRelativeScoreLabel(p1Scores, course?.pars || Array(18).fill(4));
                            return (
                      ...
                      <tr className={`text-slate-800 ${p2RowBg}`} id={`scorecard-back-p2-row-${game.id}`}>
                        ...
                        {p2Scores.slice(9, 18).map((score, idx) => (
                          <td key={idx + 9} className={`p-0.5 sm:p-1 border-b-2 ${idx === 8 ? '' : 'border-r'} border-slate-300`}>
                            <div className="flex flex-col items-center justify-center py-1 select-none">
                              <div className="w-full h-7 sm:h-8 flex items-center justify-center">
                                {renderHoleScoreBadge(score, course?.pars[idx + 9] || 4)}
                              </div>
                              {score > 0 && (() => {
                                const stats = getHoleShotAndPutt(score, idx + 9, p2.id);
                                return stats ? (
                                  <span className="text-[9px] sm:text-[10px] text-slate-400 font-mono mt-0.5 whitespace-nowrap leading-none">
                                    {stats.shots}/{stats.putts}
                                  </span>
                        ...
                        <td className={`py-1.5 px-1 text-center border-b-2 border-r-2 border-slate-400 ${p2OutInBg}`}>
                          {(() => {
                            const relLabel = getRelativeScoreLabel(p2Scores, course?.pars || Array(18).fill(4));
                            return (
                              <span className="font-mono text-xs sm:text-sm font-black text-red-600">
