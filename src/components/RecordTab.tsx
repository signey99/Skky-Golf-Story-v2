...
export const RecordTab: React.FC<RecordTabProps> = ({
  ...
  const getPlayerStats = (playerId: string) => {
    ...
    let bestScoreDates: string[] = [];
    let bestStrokesDates: string[] = [];
    let bestPuttsDates: string[] = [];
    ...
    completedGames.forEach(game => {
      ...
      if (roundSum > 0) {
        ...
        if (roundStrokes < bestStrokes) {
          ...
          bestStrokesDates = [game.date];
        } else if (roundStrokes === bestStrokes) {
          bestStrokesDates.push(game.date);
        }

        if (roundPutts < bestPutts) {
          bestPutts = roundPutts;
          bestPuttsDates = [game.date];
        } else if (roundPutts === bestPutts) {
          bestPuttsDates.push(game.date);
        }
  ...
  const formatDateMMDDYYYY = (dateVal: string | Date) => {
    try {
  ...
  const renderRecordCell = (
    ...
    if (value === '-') {
      return <span className="font-extrabold text-slate-400">-</span>;
    }

    const isActive = activeTooltip?.playerId === playerId && activeTooltip?.metric === metric;

    return (
      <div className={`inline-block ${isActive ? 'relative z-20' : 'relative'}`}>
        <button
          onClick={(e) => {
            e.stopPropagation();
            setActiveTooltip(isActive ? null : { playerId, metric });
          }}
          className={`font-extrabold ${textClass} hover:opacity-80 transition-all focus:outline-none border-b border-dashed border-slate-400/80 hover:border-slate-600 pb-0.5 cursor-pointer select-none`}
        >
          {value}
        </button>
        {isActive && (
          <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 z-30 bg-slate-900 text-white text-xs rounded-lg py-1.5 px-3 shadow-lg whitespace-nowrap animate-fade-in font-sans text-center border border-slate-700/50">
            <div className="font-mono text-xs font-semibold text-white">
              {dates.map(d => formatDateMMDDYYYY(d)).filter(Boolean).join(', ') || 'N/A'}
            </div>
  ...
  const renderTrendChart = () => {
    const gamesWithScores = completedGames.filter(game => {
      ...
      return p1RoundScore > 0 || p2RoundScore > 0;
    });
    ...
    // find min and max scores to scale Y-axis based on latest 10 games
    let allScores: number[] = [];
    const trendData = gamesWithScores.slice(-10).map(game => {
      const course = courses.find(c => c.id === game.courseId);
    ...
    return (
      <div className="space-y-2" id="trend-chart-component">
        <div className="w-full overflow-hidden" id="trend-chart-scroll">
          <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-auto overflow-visible font-sans max-h-[220px]">
            {/* Grid Lines */}
            {gridScores.map((scoreValue, i) => (
              <g key={i} className="opacity-10">
                ...
                <text 
                  x={paddingX - 10} 
                  y={getY(scoreValue) + 3.5} 
                  fontSize="10" 
                  fill="#475569" 
                  textAnchor="end"
                  className="font-mono font-medium"
                >
                  {scoreValue === 0 ? 'E' : scoreValue}
                </text>
              </g>
            ))}
            ...
            {/* Layer 2: Circle Dots (Drawn on top of lines) */}
            {trendData.map((d, idx) => (
            ...
            {/* Layer 3: Text Score Labels (Drawn on top of everything) */}
            {trendData.map((d, idx) => {
              const labels: React.ReactNode[] = [];
              const isSameScore = d.p1 === d.p2 && d.p1Raw > 0 && d.p2Raw > 0;

              if (d.p1Raw > 0) {
                let textX = getX(idx);
                let textY = getY(d.p1) - 10;

                if (isSameScore) {
                  textX = getX(idx) - 15;
                  textY = getY(d.p1) + 4;
                } else if (d.p2Raw > 0) {
                  // Dynamic separation: if Player 1 is physically HIGHER on screen (smaller getY), shift UP.
                  // Otherwise (larger getY), shift DOWN.
                  const isP1HigherOnScreen = getY(d.p1) < getY(d.p2);
                  textY = isP1HigherOnScreen ? getY(d.p1) - 10 : getY(d.p1) + 16;
                }
              ...
              if (d.p2Raw > 0) {
                let textX = getX(idx);
                let textY = getY(d.p2) + 16;

                if (isSameScore) {
                  textX = getX(idx) + 15;
                  textY = getY(d.p2) + 4;
                } else if (d.p1Raw > 0) {
                  // Dynamic separation: if Player 2 is physically HIGHER on screen (smaller getY), shift UP.
                  // Otherwise (larger getY), shift DOWN.
                  const isP2HigherOnScreen = getY(d.p2) < getY(d.p1);
                  textY = isP2HigherOnScreen ? getY(d.p2) - 10 : getY(d.p2) + 16;
                }
              ...
              return <g key={`labels-group-${idx}`}>{labels}</g>;
            })}
          </svg>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 relative" id="record-tab-container">
      {/* Click outside backdrop to dismiss active tooltip */}
      {activeTooltip && (
        <div 
      ...
      {/* Unified Tab Header */}
      <div className="sticky top-[72px] md:top-[96px] z-30 bg-slate-50/90 backdrop-blur-md -mx-1.5 px-1.5 md:-mx-4 md:px-4 py-1.5 flex items-center gap-3 border-b border-slate-200/80" id="record-header-row">
          ...
          <table className="w-full text-left border-collapse text-sm sm:text-base font-sans" id="records-table">
            ...
            <tbody className="divide-y divide-slate-200 font-mono text-slate-700" id="records-table-body">
              ...
              <tr className="bg-slate-100/65 hover:bg-slate-100 transition-colors" id="row-best-score">
                ...
                <td className="py-2.5 px-3 text-center">{renderRecordCell(p1.id, 'score', p1Stats.bestScore, p1Stats.bestScoreDates, p1TextClass)}</td>
                <td className="py-2.5 px-3 text-center">{renderRecordCell(p2.id, 'score', p2Stats.bestScore, p2Stats.bestScoreDates, p2TextClass)}</td>
              </tr>
              ...
              <tr className="bg-slate-100/65 hover:bg-slate-100 transition-colors" id="row-best-shot">
                ...
                <td className="py-2.5 px-3 text-center">{renderRecordCell(p1.id, 'shot', p1Stats.bestShot, p1Stats.bestStrokesDates, p1TextClass)}</td>
                <td className="py-2.5 px-3 text-center">{renderRecordCell(p2.id, 'shot', p2Stats.bestShot, p2Stats.bestStrokesDates, p2TextClass)}</td>
              </tr>
              ...
              <tr className="bg-slate-100/65 hover:bg-slate-100 transition-colors" id="row-best-put">
                ...
                <td className="py-2.5 px-3 text-center">{renderRecordCell(p1.id, 'put', p1Stats.bestPut, p1Stats.bestPuttsDates, p1TextClass)}</td>
                <td className="py-2.5 px-3 text-center">{renderRecordCell(p2.id, 'put', p2Stats.bestPut, p2Stats.bestPuttsDates, p2TextClass)}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 space-y-4" id="trend-card">
        <div className="flex items-center gap-2" id="trend-header">
          ...
          <h3 className="text-base font-bold text-slate-800">Recent Score Progression Trend</h3>
        </div>
        {renderTrendChart()}
      </div>
    </div>
